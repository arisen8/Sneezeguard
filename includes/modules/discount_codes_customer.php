      ...
      case 'insert':
        if (!empty($HTTP_POST_VARS['discount_codes']) && !empty($HTTP_POST_VARS['discount_values'])) {
          $sql_data_array = array('products_id' => '',
								  ...
								  $HTTP_POST_VARS['applies_to'] = 2;
			$HTTP_POST_VARS['categories_id'] = array("55","56","57","58","59","60","61","62","63","64","65","66","67","68","69","70","71","72","73","74","75","76","77","78","79","80","81","82","83","84","85","86","87","88","89","90","91","92","93","94","95","96","97","98","99","100","101","102","103","104","105","106","107","108","109","110","111","112","113");
        ...
        <td><table border="0" cellspacing="0" cellpadding="2">
          ...
          <tr>
			...
			<?php
				  ...
				  //$select_string = '<select name="categories_id[]" size="10" multiple style="width: 28px;"  disabled="true" id="categories_id">';
                  // $categories_query = tep_db_query("select categories_id from categories");
				  
				  
				  ?>
				  <td class="main" valign="bottom"><?php echo '<label>' . str_replace('<input type="checkbox"', '<input type="checkbox" id="categories"', tep_draw_checkbox_field('categories', '1')) . 'All ' . TEXT_CATEGORIES . '</label><br>' . $select_string; ?></td>
...
?>
          ...
          <tr>
            <td></td>
            <td></td>
            <td class="main"><?//php echo TEXT_EXCLUDED_PRODUCTS; ?>
            <td></td>
          ...
          <tr>
            ...
            <td></td>
<?php
    $customers_id = '<select name="customers_id[]" size="10" multiple style="width: 380px;" id="customers_id">';
    $customers_query = tep_db_query("select customers_id, concat(customers_lastname, ', ', customers_firstname, ' (', customers_email_address, ')') as customers_info from " . TABLE_CUSTOMERS . " order by customers_lastname, customers_id");
          ...
          <tr>
            <td valign="top"><table border="0" width="100%" cellspacing="0" cellpadding="2">
              <tr class="dataTableHeadingRow">
                ...
                <td class="dataTableHeadingContent"><?php echo TABLE_HEADING_COMPANY_NAME; ?></td>
...
<?php
    $discount_codes_query_raw = "select dc.discount_codes_id, dc.products_id, dc.categories_id, dc.manufacturers_id, dc.excluded_products_id, dc.customers_id, dc.orders_total, dc.order_info, dc.discount_codes, dc.discount_values, dc.minimum_order_amount, dc.expires_date, dc.number_of_orders, dc.number_of_use, dc.number_of_products, dc.status,ab.entry_company,cs.customers_firstname from discount_codes dc,address_book ab,customers cs  WHERE ab.customers_id=dc.customers_id AND dc.customers_id=cs.customers_id   order by dc.discount_codes_id desc";
...
?>
                ...
                <td class="dataTableContent"><?php echo $discount_codes['entry_company']; ?></td>
                 <td class="dataTableContent"><?php echo $discount_codes['customers_firstname']; ?></td>
    ...
    default:
      if (is_object($dInfo)) {
        ...
        if (!empty($dInfo->customers_id)) {
          $select_string = '';
          $customers_query = tep_db_query("select concat(customers_lastname, ', ', customers_id, ' (', customers_email_address, ')') as customers_info from " . TABLE_CUSTOMERS . " where customers_id in (" . $dInfo->customers_id . ") order by customers_lastname, customers_id");
          while ($customers = tep_db_fetch_array($customers_query)) {
