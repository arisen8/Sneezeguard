 ...
 // - Zappo - Option Types v2 - Use some easy shorter names for products_options_name values
  ...
  if ($products_attribs_array['options_values_price'] != '0') {
    ...
    case OPTIONS_TYPE_RADIO:
      $tmp_html = '';
      $products_options_query = tep_db_query("select pov.products_options_values_id, pov.products_options_values_name, pa.options_values_price, pa.price_prefix from " . TABLE_PRODUCTS_ATTRIBUTES . " pa, " . TABLE_PRODUCTS_OPTIONS_VALUES . " pov where pa.products_id in ". $products_ids ." and pa.options_id = '" . $ProdOpt_ID . "' and pa.options_values_id = pov.products_options_values_id and pov.language_id = '" . $languages_id . "' order by pa.products_options_sort_order");
      ...
      $tmp_html .= '<tr><td width="207" colspan="2" align="left" class="linkborder">';
        $tmp_html .= tep_draw_radio_field('id[' . $ProdOpt_ID . ']', $products_options_array['products_options_values_name'], $checked,'id="cmbooption_'.$opt_count.'" onClick="getPrice('.(int)$HTTP_GET_VARS['id'].','. $ProdOpt_ID.','.$products_options_array['products_options_values_id'].')"');
        ...
        } [ hide price from string ] */
        $tmp_html .= '</td></tr>';
      }
      $tmp_html .= ''; ?>
      <tr>
        <td align="left" colspan="2" class="heading" class="main"><?php echo $tmp_html;  ?></td>
      </tr>
       <?php
    break;
    ...
    case OPTIONS_TYPE_CHECKBOX:
      ...
      $tmp_html = tep_draw_checkbox_field('id[' . $ProdOpt_ID . ']', $products_attribs_array['options_values_id'], $checked,'id="cmbooption_'.$opt_count.'"') . ' &nbsp; ';
      $tmp_html .= $ProdOpt_Comment ;
      $tmp_html .= $tmp_html_price; ?>
      <tr>
        <td align="left" colspan="2" class="heading"><?php echo $ProdOpt_Name . ' :'; ?><?php echo $tmp_html;  ?></td>
      </tr> <?php
    break;
