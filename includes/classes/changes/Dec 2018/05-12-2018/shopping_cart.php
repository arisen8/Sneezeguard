...
// products price
        $product_query = tep_db_query("select products_id, products_price, products_tax_class_id, products_model, products_weight from " . TABLE_PRODUCTS . " where products_id = '" . (int)$products_id . "'");
        if ($product = tep_db_fetch_array($product_query)) {
          $prid = $product['products_id'];
          $products_tax = tep_get_tax_rate($product['products_tax_class_id']);
		  //for 20% discount on glass
		  $glss=$product['products_model'];
			if(strpos($glss,'GL') !== false||strpos($glss,'Glass') !== false){
				$products_price = $product['products_price']-$product['products_price']*20/100;
		  }
			else{
		...
		$ct1=0;
          while($ct1<sizeof($_SESSION['product_final'])){
            if($_SESSION['product_final'][$ct1]['id']==$prid){
    ...
    function get_products() {
      ...
      while (list($products_id, ) = each($this->contents)) {
        ...
        if ($products = tep_db_fetch_array($products_query)) {
		  ...
		  //for 20% discount on glass
		  $glss=$products['products_model'];
			if(strpos($glss,'GL') !== false||strpos($glss,'Glass') !== false){
          $products_price = $products['products_price']-$products['products_price']*20/100;
			}
			else{
