...
/*uncomment follow if found any issue in shipping*/
$shippingiddd=$_SESSION['shippingiddd'];
$shippingtitle=$_SESSION['shippingtitle'];
$shippingcost=$_SESSION['shippingcost'];
//echo'<b style="color:red"><pre>'; print_r($_SERVER['REQUEST_URI']); echo'</b>';
if (empty($shipping)) {
	if((strpos($_SERVER['REQUEST_URI'],'shopping_cart') !== false))
	 {
		 
	 }
	 else{
  $shipping=array('id'=>$shippingiddd, 'title'=>$shippingtitle, 'cost'=>$shippingcost);
	 }
}
      ...
      for ($i=0, $n=sizeof($products); $i<$n; $i++) {
		...
		$ct1=0;
		 ...
		 //print_r($this->products[$index]['final_price']*$this->products[$index]['qty']);echo'<br />';
		// print_r($_SESSION['product_final']);echo'<br />';
          while($ct1<sizeof($_SESSION['product_final'])){
            // echo $ct1;
            if($_SESSION['product_final'][$ct1]['id']==$this->products[$index]['id']){
        ...
        if( is_object( $this->coupon ) ) {
          ...
          $shown_price = $shown_price['actual_shown_price'];
			...
			$finalprices_with_ctt= round($this->products[$index]['final_price']*$ct);
          $shown_price = tep_add_tax($finalprices_with_ctt, $this->products[$index]['tax']) * $this->products[$index]['qty'];
		 // echo$ct; echo'-'; print_r($shown_price);echo'<br />';
		 
          $this->info['subtotal'] += $shown_price;
		  //echo'<br />';
		 // print_r($this->products[$index]);echo'<br />';
        }
        /**************
        ...
        **************/
		...
		//echo$shown_price = $currencies->calculate_price($this->products[$index]['final_price'], $this->products[$index]['tax'], $this->products[$index]['qty']);echo'<br />';
        //$this->info['subtotal'] += $shown_price;
		//echo$this->info['subtotal'];echo'<br />';
