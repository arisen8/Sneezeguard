...
<!-- <atom:link href="https://www.sneezeguard.com/rss.php?language=en" rel="self" type="application/rss+xml" /> -->
<!--Remove comments if you want this to remain -->
...
<image>
  <url><?php echo $weblink . DIR_WS_CATALOG.'images/rss_logo.jpg';?></url> <!-- upload and replace name of your logo file -->
  <title><?php/* echo utf8_encode($rss_title); */?><?php echo utf8_encode($store_name); ?></title>
  <link><?php echo $weblink;?></link>
  <description><?php echo utf8_encode($description); ?></description>
  <width>100</width>   <!-- add your logo's width (px) then remove this comment -->
  <height>120</height>  <!-- add your logo's height (px) then remove this comment -->
  <!-- <width>YOUR LOGO WIDTH PIXELS</width>   add your logo's width (px) then remove this comment -->
  <!-- <height>YOUR LOGO HEIGHT PIXELS</height>  add your logo's height (px) then remove this comment -->
</image>
<copyright>Copyright 2009, YourWebsite.com CHANGE TO YOUR COPYRIGHT NOTICE</copyright>  <!-- add your copyright info then remove this comment -->
...
while( $row = tep_db_fetch_array($query) ){
  ...
  $link = tep_href_link(FILENAME_PRODUCT_INFO1, 'products_id=' . $id, 'NONSSL', false);
...
$price = sprintf("%01.2f", $price);
  ...
  $buy_link = tep_href_link(FILENAME_PRODUCT_INFO1, tep_get_all_get_params(array('action')) . 'action=buy_now&products_id=' . $id);
  // Encoding to UTF-8
  ...
  echo '  <link>' . $link . '</link>' . "\n";
  //echo '<brand><%# EvalCDATA("ADM Sneezeguard", true) %></brand>';
  // echo'<brand><% EvalCDATA("SKU.SKUManufacturer.ManufacturerDisplayName", true) %></brand>'."\n";
  echo '  <description>'; // . "\n";
  if ($ecommerce=='' && $image != '') {
    echo '<![CDATA[<a href="' . $link . '"><img src="' . $image_url . '</a><br />]]>';
  }
  echo $desc;
  if ($ecommerce=='') {
    echo '<![CDATA[<br><b>Manufacturer:&nbsp;</b>ADM Sneezeguards' . $manufacturer . ']]>' . "\n";  // delete this line if you do not want to list manufacturer 
...
file_put_contents('rssfeed.xml', ob_get_contents());
	/*
