...
if($row["Name"]=='EP8')
{
  $linkss=  'Model='.$row["Name"].'&pid=133';
}
elseif($row["Name"]=='EP7')
{
  $linkss=  'Model='.$row["Name"].'&pid=130';
}
else{
    $linkss=   'Model='.$row["Name"].'&mid='.$category_list_array[$j++].'';
      ...
      echo('<A HREF="'.tep_href_link('info.php', $linkss).'"><IMG alt="sneeze guard '.$row["Name"].'" title="'.$row["Name"].'" SRC="images/Models/'.$row["Name"].'.jpg" BORDER=0>' . '</A>');  
